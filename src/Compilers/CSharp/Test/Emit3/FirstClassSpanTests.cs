...
public class FirstClassSpanTests : CSharpTestBase
{
    ...
    [Fact]
    public void OverloadResolution_ReadOnlySpanVsArray_ExpressionTree_04()
    {
        var source = """
            using System;
            using System.Linq.Expressions;
        ...
        var expectedDiagnostics = new[]
        {
            // (4,13): error CS0121: The call is ambiguous between the following methods or properties: 'C.M(object[])' and 'C.M(ReadOnlySpan<object>)'
            // C.R(() => C.M(null));
            Diagnostic(ErrorCode.ERR_AmbigCall, "M").WithArguments("C.M(object[])", "C.M(System.ReadOnlySpan<object>)").WithLocation(4, 13),
            // (5,13): error CS0121: The call is ambiguous between the following methods or properties: 'C.M(object[])' and 'C.M(ReadOnlySpan<object>)'
            // C.R(() => C.M(default));
            Diagnostic(ErrorCode.ERR_AmbigCall, "M").WithArguments("C.M(object[])", "C.M(System.ReadOnlySpan<object>)").WithLocation(5, 13)
        };
