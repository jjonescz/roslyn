...
public class FirstClassSpanTests : CSharpTestBase
{
    ...
    [Theory, CombinatorialData]
    public void BreakingChange_TypeInference_SpanVsIEnumerable_01_Workaround_01(
    ...
    [Theory, CombinatorialData]
    public void BreakingChange_TypeInference_SpanVsIEnumerable_01_Workaround_02(
        [CombinatorialValues("Span", "ReadOnlySpan")] string type)
    {
        var source = $$"""
            using System;
            ...
            static class E
            {
                [OverloadResolutionPriority(-1)] public static void R<T>(this {{type}}<T> s) => Console.Write(1);
                public static IEnumerable<T> R<T>(this IEnumerable<T> e) { Console.Write(2); return e; }
                public static void R2<T>(this {{type}}<T> s) => Console.Write(3);
            }
            """;
        var comp = CreateCompilationWithSpanAndMemoryExtensions([source, OverloadResolutionPriorityAttributeDefinition]);
        CompileAndVerify(comp, expectedOutput: "2313").VerifyDiagnostics();
    }
