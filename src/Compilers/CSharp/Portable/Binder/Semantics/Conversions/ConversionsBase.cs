        ...
        private static LambdaConversionResult IsAnonymousFunctionCompatibleWithDelegate(UnboundLambda anonymousFunction, TypeSymbol type, CSharpCompilation compilation, bool isTargetExpressionTree)
        {
            ...
            if (anonymousFunction.HasSignature)
            {
                ...
                if (anonymousFunction.HasExplicitlyTypedParameterList)
                {
                    ...
                    {
                        var delegateRefKind = delegateParameters[p].RefKind;
                        var anonymousFunctionRefKind = anonymousFunction.RefKind(p);
                        if ((delegateRefKind != anonymousFunctionRefKind &&
                            !OverloadResolution.IsRefMismatchAcceptableForMethodConversion(delegateRefKind, anonymousFunctionRefKind, compilation)) ||
