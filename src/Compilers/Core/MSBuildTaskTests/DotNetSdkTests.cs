...
some_prop = some_val");
  ...
  <ItemGroup>
    <GlobalAnalyzerConfigFiles Include=""mycustom.config"" />
        ...
        {
            ProjectDir.CreateFile("MyGenerator.csproj").WriteAllText("""
                <Project Sdk="Microsoft.NET.Sdk">
                    <PropertyGroup>
                        <TargetFramework>netstandard2.0</TargetFramework>
            ...
            VerifyValues(
                ...
                <ItemGroup>
                    <ProjectReference Include=".\MyGenerator.csproj" OutputItemType="Analyzer" ReferenceOutputAssembly="false" />
                </ItemGroup>
                """,
                customTargets: null,
                targets: new[] { "Restore", "CoreCompile" },
                expressions: new[] { "@(Analyzer)" },
                expectedResults: new[] { "" },
                additionalMsbuildArguments: "/p:RuntimeIdentifier=win-arm64 /p:PublishSingleFile=True");
        }
    }
}
