...
internal sealed class FindDerivedTypesTgrepAlgorithm : QueryAlgorithm<FindDerivedTypesQuery>
{
    public override string Name => "find-derived-types-tgrep";
    ...
    {
        var summary = $"tgrep search for '(class|record|struct) \\w+ ... {query.Name}'";
        var searchResult = await context.SearchDerivedTypesTgrepAsync(query.Name, cancellationToken);
